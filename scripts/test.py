...
from boxcars_image_transformations import (add_bb_noise_flip, alter_HSV,
                                           image_drop, unpack_3DBB)
...
if model is None:
    ...
    model = Model(input=base_model.input, output=predictions, name="%s%s" % (
        args.train_net, {True: "_estimated3DBB", False: ""}[args.estimated_3DBB is not None]))
...
if args.test is not None:
    ...
    print('real type is'
          ' [{}]'.format(
              dataset.get_name_of_classes(
                  np.nonzero(dataset.Y["test"][idx])[0][0])
          )
          )
...
pred_prob = pred[0, pred_cls]
#  dataset.get_name_of_classes(np.argmax(pred, axis=1))
#  pred_cls = np.argmax(pred, axis=1)[0]
print("pred type is [{}], prob={}".format(
    dataset.get_name_of_classes(pred_cls), pred_prob))
print(" -- Image processing time: %.1fms" %
      ((end_time - start_time) / eval_steps * 1000))
